...
This is software that controls insulin delivery. Safety is not optional.

- **Fail-safe defaults** — unknown state = do NOT deliver insulin
- **Command confirmation** — every delivery command verified by pod response
- **Audit trail** — append-only log of all commands with timestamps and checksums
- **Timeout enforcement** — 8s command timeout, 15s connection timeout
- **Max bolus guard** — configurable limit (default 30U)
- **Stale data detection** — values show age, warn if outdated
- **PIN/biometric gate** — bolus delivery requires authentication
...
> **Provenance.** OpenPod is an independent reimplementation informed by reverse engineering for interoperability and research purposes. The implementation is source-original. Protocol behavior and compatibility details were derived through reverse engineering. The repository does not include proprietary vendor binaries or source code. Cryptographic implementations are based on published standards (X25519, AES-CCM, EAP-AKA/MILENAGE per 3GPP TS 35.206).
