...
static int host_rcv_pkt(uint8_t *data, uint16_t len)
{
    ...
    /* Raw hex dump of first 20 bytes for debugging */
    if (g_cb_count <= 20 || g_cb_count % 500 == 0) {
        printf("DBG[%lu] len=%d raw: ", (unsigned long)g_cb_count, len);
    ...
    /* Try both with and without H4 header — some VHCI implementations
     * strip the H4 type byte, others include it */
    uint8_t evt_code;
    uint8_t evt_len;
    int hdr_offset;
    ...
    if (evt_code == HCI_EVT_LE_META && evt_len > 0) {
        ...
        if (sub_event == HCI_LE_ADV_REPORT) {
            ...
            ESP_LOGI(TAG, "LE meta sub-event: 0x%02X (len=%d)", sub_event, evt_len);
        }
    }
    else if (evt_code == HCI_EVT_COMMAND_COMPLETE) {
        if (evt_len >= 3) {
            uint16_t opcode = data[hdr_offset + 1] | (data[hdr_offset + 2] << 8);
            uint8_t status = data[hdr_offset + 3];
            ESP_LOGI(TAG, "HCI cmd complete: opcode=0x%04X status=%d", opcode, status);
        }
    }
    else {
        ESP_LOGI(TAG, "HCI event: code=0x%02X len=%d", evt_code, evt_len);
    }
