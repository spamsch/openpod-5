...
def build_sp_value(name: str, value: bytes) -> bytes:
    """Build a binary SP command: NAME=<2-byte-BE-len><value>."""
    import struct
    return name.encode("ascii") + b"=" + struct.pack(">H", len(value)) + value
...
class TestTwiHeader:
    ...
    def test_build_twi_roundtrip(self) -> None:
        ...
        header = BleTransportProtocol._build_twi_header(
            3, len(payload), CONTROLLER_ID, POD_ID
        )
        data = header + payload
        result = BleTransportProtocol._strip_twi_header(data)
        assert result is not None
        ...
        # cmd_count is byte5:byte6 = (payload_length << 5) >> 8 = length >> 3
        assert meta["cmd_count"] == len(payload) >> 3
        assert rhp == payload

    def test_build_twi_p0_reply_length_encoding(self) -> None:
        """Regression: P0= reply (6 bytes) must encode length correctly.

        Earlier the builder clobbered byte 7's upper 3 bits with the
        request flag classifier, causing 6-byte payloads to encode as
        length=7 on the wire. The PDM dropped the frame and pairing
        timed out post-P0.
        """
        # encode_sp("P0=", b"\xa5") = b"P0=\x00\x01\xa5", 6 bytes
        payload = b"P0=\x00\x01\xa5"
        assert len(payload) == 6
        header = BleTransportProtocol._build_twi_header(
            4, len(payload), CONTROLLER_ID, POD_ID, flags=0xE0,
        )
        # Decoded length must equal actual payload length, not 7.
        encoded = (header[6] << 8) | header[7]
        assert (encoded >> 5) == 6, (
            f"length encoded as {encoded >> 5}, expected 6 "
            f"(byte6=0x{header[6]:02x} byte7=0x{header[7]:02x})"
        )
...
class TestSpCommandParsing:
    ...
    def test_parse_unknown_prefix_treated_as_text(self) -> None:
        """Unknown prefixes are returned as plain-text commands."""
        payload = b"FOO=bar,SP1=" + b"\x00\x02\xaa\xbb"
        commands = BleTransportProtocol.parse_sp_commands(payload)
        assert len(commands) == 2
        assert commands[0][0] == "FOO=bar"  # plain text
        assert commands[1][0] == "SP1"
        assert commands[1][1] == b"\xaa\xbb"
