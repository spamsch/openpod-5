...
def phone_compute_confirmation(
    ...
    """
    Compute the phone-side AES-CCM confirmation value.

    Phone is role=2 (controller).
    Nonce (13 bytes): 0x02 || phone_nonce[0:6] || pod_nonce[0:6]
    Plaintext: phone_pubkey || pod_pubkey
    """
    nonce = bytes([0x02]) + phone_nonce[:6] + pod_nonce[:6]
    plaintext = phone_pub + pod_pub
...
class TestPairing:
    ...
    def test_full_pairing_flow(self, session: ProtocolSession):
        ...
        # AES-CCM confirmation: 64-byte ciphertext + 8-byte tag = 72 bytes
        pod_conf = conf_response[2:]
        assert len(pod_conf) == 72
        ...
        # Step 4: Phone verifies pod's confirmation by decrypting
        pod_ccm_nonce = bytes([0x01]) + pod_nonce[:6] + phone_kp.nonce[:6]
        pod_plaintext = aes_ccm.decrypt(
            key=phone_derived.confirmation_key,
            ciphertext=pod_conf,
            nonce=pod_ccm_nonce,
            tag_length=8,
        )
        assert pod_plaintext == pod_pub + phone_kp.public_key_bytes
    ...
    def test_wrong_confirmation_fails(self, session: ProtocolSession):
        ...
        # Send bogus confirmation (needs >= 8 bytes for AES-CCM tag)
        bogus_conf = bytes(72)
...
class TestEapAka:
    ...
    def test_eap_aka_authentication(self, session: ProtocolSession):
        ...
        assert eap_payload[8] == AT_RES
        assert eap_payload[9] == 0x03
    ...
    def test_empty_active_session_payload_replays_cached_aka_response(
        ...
        sqn = b"\x00" * 6
        challenge_msg = phone_build_eap_aka_challenge(ltk, rand, sqn)
        first_response = session.on_message(challenge_msg)

        assert first_response is not None
        assert first_response[0] == MSG_EAP
        assert session.phase == SessionPhase.ACTIVE

        replay = session.on_twi_session_message(b"")

        assert replay == first_response[1:]
        assert replay[8] == AT_RES
        assert replay[9] == 0x03
...
class TestEndToEndBolusFlow:
    ...
    def test_full_flow_init_to_bolus(self, pod_state: PodState):
        ...
        pod_conf = conf_resp[2:]
        # AES-CCM: 64-byte ciphertext + 8-byte tag
        assert len(pod_conf) == 72
        ...
        # Verify pod's confirmation by decrypting
        pod_ccm_nonce = bytes([0x01]) + pod_nonce[:6] + phone_kp.nonce[:6]
        pod_plaintext = aes_ccm.decrypt(
            key=phone_derived.confirmation_key,
            ciphertext=pod_conf,
        ...
        # Phone computes and sends its AES-CCM confirmation
        phone_conf = phone_compute_confirmation(
            phone_derived.confirmation_key,
            phone_kp.nonce,
            pod_nonce,
...
# ---------------------------------------------------------------------------
# SPS0 certificate rejection tests
# ---------------------------------------------------------------------------


class TestSps0CertRejection:
    """Test that certificate algorithms are rejected in SPS0."""

    def test_rejects_cert_algorithm_0x09(self, session: ProtocolSession):
        """SPS0 with 0x09 (P-256 + cert) should be rejected."""
        # Init first to set up pairing state
        session.on_message(phone_build_init())
        assert session.phase == SessionPhase.PAIRING

        # SPS0 value: selector(2) + algorithm(1)
        sps0_value = bytes([0x01, 0x00, 0x09])
        response = session.on_twi_commands([("SPS0", sps0_value)])

        assert response is not None
        # Response should contain SPS0= with status 0x02 (ALGO_NOT_SUPPORTED)
        assert response.startswith(b"SPS0=")
        # Parse: "SPS0=" + length(2) + version(1) + status(1) + algo(1) + crc(2)
        inner = response[7:]  # skip "SPS0=" + 2 length bytes
        assert inner[1] == 0x02  # ALGO_NOT_SUPPORTED
        assert inner[2] == 0x09  # echoes the algorithm

    def test_rejects_cert_algorithm_0x08(self, session: ProtocolSession):
        """SPS0 with 0x08 (X25519 + cert) should be rejected."""
        session.on_message(phone_build_init())

        sps0_value = bytes([0x01, 0x00, 0x08])
        response = session.on_twi_commands([("SPS0", sps0_value)])
    ...
    def test_rejects_cert_algorithm_0x0D(self, session: ProtocolSession):
        """SPS0 with 0x0D (P-256 + pwd + cert) should be rejected."""
        session.on_message(phone_build_init())

        sps0_value = bytes([0x01, 0x00, 0x0D])
        response = session.on_twi_commands([("SPS0", sps0_value)])
    ...
    def test_accepts_non_cert_algorithm_0x00(self, session: ProtocolSession):
        """SPS0 with 0x00 (X25519, no cert) should be accepted."""
        session.on_message(phone_build_init())

        sps0_value = bytes([0x01, 0x00, 0x00])
        response = session.on_twi_commands([("SPS0", sps0_value)])

        assert response is not None
        inner = response[7:]
        assert inner[1] == 0x00  # ACCEPTED
        assert inner[2] == 0x00

    def test_accepts_non_cert_algorithm_0x01(self, session: ProtocolSession):
        """SPS0 with 0x01 (P-256, no cert) should be accepted."""
        session.on_message(phone_build_init())

        sps0_value = bytes([0x01, 0x00, 0x01])
        response = session.on_twi_commands([("SPS0", sps0_value)])
    ...
    def test_rejection_can_be_disabled(self, pod_state: PodState):
        """With reject_cert_algorithms=False, 0x09 is accepted."""
        session = ProtocolSession(
            pod_state=pod_state,
            firmware_id=FIRMWARE_ID,
            ecdh_seed=ECDH_SEED,
            reject_cert_algorithms=False,
        )
        session.on_message(phone_build_init())
...
# ---------------------------------------------------------------------------
# TWI-path pairing (SP1 → SP2 → SPS0 → SPS1 → SPS2)
# ---------------------------------------------------------------------------


class TestTwiPairing:
    """Test the full TWI-path pairing flow using SPS commands."""

    def test_full_twi_pairing_flow(self, session: ProtocolSession):
        """Complete pairing via TWI commands (the production PDM path)."""
        ...
        # Step 2: SP1 (controller ID registration)
        sp1_value = CONTROLLER_ID
        resp = session.on_twi_commands([("SP1", sp1_value)])
        assert resp == b"ESSP1.0=0"

        # Step 3: SP2 (device capabilities)
        sp2_value = CONTROLLER_ID + bytes([0x00, 0x01, 0x00, 0x00, 0x00])
        crc = crc16_ccitt(sp2_value)
        sp2_value += crc.to_bytes(2, "big")
        resp = session.on_twi_commands([("SP2", sp2_value)])
        assert resp == b"ESSP2.0=0"

        # Step 4: SPS0 (algorithm negotiation — 0x00, no cert)
        sps0_value = bytes([0x01, 0x00, 0x00])
        resp = session.on_twi_commands([("SPS0", sps0_value)])
        assert resp is not None
        assert resp.startswith(b"SPS0=")
        # Verify accepted
        inner = resp[7:]
        assert inner[1] == 0x00  # ACCEPTED

        # Step 5: SPS1 (key exchange)
        phone_kp = EcdhKeyPair()
        sps1_value = phone_kp.public_key_bytes + phone_kp.nonce
        resp = session.on_twi_commands([("SPS1", sps1_value)])
        assert resp is not None
        assert resp.startswith(b"SPS1=")

        # Parse pod's SPS1 response: "SPS1=" + length(2) + pubkey(32) + nonce(16)
        sps1_payload = resp[7:]  # skip "SPS1=" + 2 length bytes
        sps1_pod_pub = sps1_payload[:32]
        sps1_pod_nonce = sps1_payload[32:48]
        # Pod may have regenerated keys if algorithm changed, so use SPS1 keys
        # (for algorithm 0x00 with no change, they should match init response)
        assert sps1_pod_pub == pod_pub
        assert sps1_pod_nonce == pod_nonce

        # Step 6: Phone computes shared secret and keys
        shared_secret = phone_kp.compute_shared_secret(sps1_pod_pub)
        phone_derived = derive_keys_for_role(
            role=0,
            firmware_id=FIRMWARE_ID,
            controller_id=CONTROLLER_ID,
            local_public_key=phone_kp.public_key_bytes,
            peer_public_key=sps1_pod_pub,
            shared_secret=shared_secret,
        )

        # Step 7: SPS2 (confirmation exchange)
        phone_conf = phone_compute_confirmation(
            phone_derived.confirmation_key,
            phone_kp.nonce,
            sps1_pod_nonce,
        ...
        resp = session.on_twi_commands([("SPS2", phone_conf)])
        assert resp is not None
        assert resp.startswith(b"SPS2=")
        assert session.phase == SessionPhase.AUTHENTICATING

        # Verify pod's confirmation
        sps2_pod_conf = resp[7:]  # skip "SPS2=" + 2 length bytes
        pod_ccm_nonce = (
            bytes([0x01]) + sps1_pod_nonce[:6] + phone_kp.nonce[:6]
        )
        pod_plaintext = aes_ccm.decrypt(
            key=phone_derived.confirmation_key,
            ciphertext=sps2_pod_conf,
            nonce=pod_ccm_nonce,
            tag_length=8,
        )
        assert pod_plaintext == sps1_pod_pub + phone_kp.public_key_bytes

        # Both sides should have the same LTK
        assert phone_derived.ltk is not None

    def test_sps2_indexed_same_as_unindexed(self, session: ProtocolSession):
        """SPS2.0= should behave identically to SPS2=."""
        from omnipod_emulator.crypto.crc16 import crc16_ccitt

        # Init + SPS0 + SPS1
        init_resp = session.on_message(phone_build_init())
        sps0_value = bytes([0x01, 0x00, 0x00])
        session.on_twi_commands([("SPS0", sps0_value)])

        phone_kp = EcdhKeyPair()
        sps1_value = phone_kp.public_key_bytes + phone_kp.nonce
        resp = session.on_twi_commands([("SPS1", sps1_value)])
        sps1_pod_pub = resp[7:39]
        sps1_pod_nonce = resp[39:55]

        # Derive phone-side keys
        shared_secret = phone_kp.compute_shared_secret(sps1_pod_pub)
        phone_derived = derive_keys_for_role(
            role=0,
            firmware_id=FIRMWARE_ID,
        ...
        # Send confirmation as SPS2.0 (indexed form)
        phone_conf = phone_compute_confirmation(
            phone_derived.confirmation_key,
            phone_kp.nonce,
            sps1_pod_nonce,
        ...
        resp = session.on_twi_commands([("SPS2.0", phone_conf)])
        assert resp is not None
        assert resp.startswith(b"SPS2=")  # index 0 → no ".0" in response
        assert session.phase == SessionPhase.AUTHENTICATING

    def test_sps0_rejects_password_algorithm_0x05(self, session: ProtocolSession):
        """SPS0 with 0x05 (P-256 + password, no cert) should be rejected."""
        session.on_message(phone_build_init())

        sps0_value = bytes([0x01, 0x00, 0x05])
        response = session.on_twi_commands([("SPS0", sps0_value)])
...
# ---------------------------------------------------------------------------
# SP0/GP0 → P0 finalization
# ---------------------------------------------------------------------------


class TestPairingFinalization:
    """Test SP0/GP0 finalization responding with P0=0xa5."""

    def test_sp0_responds_with_p0(self, session: ProtocolSession):
        """SP0 should respond with P0=0xa5."""
        session.on_message(phone_build_init())

        response = session.on_twi_commands([("SP0", b"")])
        assert response is not None
        assert response.startswith(b"P0=")
        # Value should be 0xa5
        from omnipod_emulator.protocol.sp_codec import decode_sp
        value = decode_sp(response, name_offset=3)
        assert value == b"\xa5"

    def test_sp0_gp0_combined(self, session: ProtocolSession):
        """SP0,GP0 should produce one P0 response (GP0 has no response)."""
        session.on_message(phone_build_init())

        response = session.on_twi_commands([("SP0", b""), ("GP0", b"")])
        assert response is not None
        # Only P0 response, no GP0 response
        assert response.startswith(b"P0=")
        assert b"," not in response  # single response
...
class TestNonceDirectionBit:
    """Test that AES-CCM nonces include the direction bit."""

    def test_nonce_with_iv_prefix_pod_receiving(self):
        """Pod-receiving nonce should have high bit clear in sqn[0]."""
        from omnipod_emulator.crypto.eap_aka import SessionKeys

        pod_state = PodState()
        session = ProtocolSession(
            pod_state=pod_state, firmware_id=FIRMWARE_ID, ecdh_seed=ECDH_SEED,
        )
        session._session_keys = SessionKeys(
            encryption_key=bytes(16),
            controller_iv=b"\x01\x02\x03\x04",
            node_iv=b"\x05\x06\x07\x08",
        )

        nonce = session._build_nonce(
            counter=1, suffix=b"\x00" * 4, pod_receiving=True,
        )
        assert len(nonce) == 13
        # Prefix should be controller_iv + node_iv
        assert nonce[:8] == b"\x01\x02\x03\x04\x05\x06\x07\x08"
        # High bit of sqn[0] should be clear (pod receiving)
        assert nonce[8] & 0x80 == 0

    def test_nonce_with_iv_prefix_pod_sending(self):
        """Pod-sending nonce should have high bit set in sqn[0]."""
        from omnipod_emulator.crypto.eap_aka import SessionKeys
        ...
        nonce = session._build_nonce(
            counter=1, suffix=b"\x00" * 4, pod_receiving=False,
        )
        assert len(nonce) == 13
        assert nonce[:8] == b"\x01\x02\x03\x04\x05\x06\x07\x08"
        # High bit of sqn[0] should be set (pod sending)
        assert nonce[8] & 0x80 == 0x80

    def test_nonce_fallback_without_ivs(self):
        """Without IVs, nonce falls back to legacy counter+suffix format."""
        pod_state = PodState()
        session = ProtocolSession(
            pod_state=pod_state, firmware_id=FIRMWARE_ID, ecdh_seed=ECDH_SEED,
        )
        # No IV exchange — session_keys is None or has empty IVs

        nonce = session._build_nonce(
            counter=42, suffix=b"\xaa\xbb\xcc\xdd", pod_receiving=True,
        )
        assert len(nonce) == 13
        # Legacy format: counter(8) + suffix(4) + 0x00
        assert nonce[8:12] == b"\xaa\xbb\xcc\xdd"
        assert nonce[12:13] == b"\x00"
