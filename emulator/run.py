...
from omnipod_emulator.debug_bridge import DebugBridge, SessionSharedSecretProvider
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--state-file",
        default=os.environ.get("OPENPOD_STATE_FILE"),
        help=(
            "Path to a JSON file used to persist LTK + identity across "
            "restarts so the PDM can reconnect without re-pairing. "
            "Created on first successful pairing. May also be set via "
            "the OPENPOD_STATE_FILE environment variable."
        ),
    )
    parser.add_argument(
        "--key-id",
        default=os.environ.get("OPENPOD_KEY_ID", "default"),
        help=(
            "Identity key id stored in --state-file. Use distinct ids "
            "(e.g. 'pod-a', 'pod-b') with separate state files when "
            "running multiple pod identities. Default: 'default'. "
            "Env: OPENPOD_KEY_ID."
        ),
    )
    parser.add_argument(
    ...
    # ---- Debug FGH-bypass bridge ----------------------------------------
    # Opt-in HTTP sidecar used by the Elmo host-side runner to service
    # ``TwiSecPair.calcConfValue`` interceptions for v6.9.8 reverse-eng work.
    # Disabled by default because it opens a local TCP port; enable
    # explicitly on the dev Pi.
    parser.add_argument(
        "--bridge",
        action="store_true",
        default=False,
        help=(
            "Enable the debug FGH-bypass HTTP bridge (off by default). "
            "Serves /app_conf for the Elmo host-side runner."
        ),
    )
    parser.add_argument(
        "--bridge-host",
        default="127.0.0.1",
        help=(
            "Interface the debug bridge binds to. Default: 127.0.0.1 "
            "(localhost only). Use the Pi's LAN address only if the "
            "Elmo runner is on another machine."
        ),
    )
    parser.add_argument(
        "--bridge-port",
        type=int,
        default=9997,
        help="Debug bridge TCP port. Default: 9997.",
    )
...
def main() -> None:
    ...
    if args.state_file:
        logger.info(
            "Pairing state file: %s (key_id=%s)",
            args.state_file, args.key_id,
        )
    ...
    # Optional debug FGH-bypass HTTP bridge (opt-in, bound narrowly).
    bridge: DebugBridge | None = None
    if args.bridge:
        provider = SessionSharedSecretProvider(session)
        bridge = DebugBridge(provider)
        try:
            bridge.start(host=args.bridge_host, port=args.bridge_port)
        except OSError as exc:
            logger.error(
                "Debug bridge failed to bind %s:%d — %s",
                args.bridge_host, args.bridge_port, exc,
            )
            bridge = None
...
def _run_ble(
    ...
    *,
    ...
    ble_server = OmnipodBleServer(
        ...
        on_twi_session_message=session.on_twi_session_message,
