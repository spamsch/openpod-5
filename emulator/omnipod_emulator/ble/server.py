...
class OmnipodBleServer:
    ...
    async def _start_advertising(self) -> None:
        ...
        # Build manufacturer-specific data (AD Type 0xFF) to match real pod format:
        #   company_id (2 bytes LE) + 2 padding bytes +
        #   pod_id_adj_bits (1 byte, bits 4-5) +
        #   alarm_code (1 byte) + alert_code (1 byte)
        # Real pod example: 60 03 | 00 02 | 30 | 00 | 00  (7 bytes total)
        from .constants import INSULET_COMPANY_ID

        pod_id_adj = (POD_ID & 0x03) << 4  # lower 2 bits → bits 4-5
        alarm_code = 0x00
        alert_code = 0x00
        company_id = INSULET_COMPANY_ID.to_bytes(2, "little")
        mfr_data = company_id + bytes([0x00, 0x02, pod_id_adj, alarm_code, alert_code])

        # Device name matches real pod format: "AP {POD_ID_HEX} {SERIAL}"
        device_name = f"AP {POD_ID:08X} {POD_SERIAL}"

        # Advertising data: Flags + 128-bit UUID + manufacturer data
        # Scan response: device name (to fit within 31-byte ad payload limit)
